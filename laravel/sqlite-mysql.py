...
def map_sqlite_to_mysql_type(sqlite_type_raw, is_primary_key=False, is_unique=False):
    ...
    if "INT" in sqlite_type:
        ...
        elif "BIGINT" in sqlite_type: return "BIGINT"
        return "INT"
...
def migrate_sqlite_to_mysql(sqlite_db_path, mysql_config):
        ...
        for table_name_tuple in tables:
            ...
            for col in columns:
                ...
                if default_value is not None:
                    ...
                    if table_name == 'api_key' and col_name in ['created_at', 'updated_at']:
            ...
            if rows:
                ...
                for row_data in rows:
                    ...
                    # For 'api_key' specifically, ensure 'created_at' is populated
                    if table_name == 'api_key':
                        try:
                            # Find index of 'created_at' and 'updated_at' in the original columns
                            created_at_idx = original_col_names.index('created_at')
                            updated_at_idx = original_col_names.index('updated_at')

                            # Get original values, convert to string if they are not (e.g., if null or integer)
                            original_created_at = row_data[created_at_idx]
                            original_updated_at = row_data[updated_at_idx] if updated_at_idx < len(row_data) else None

                            # If original created_at/updated_at are NULL or not valid datetime strings,
                            # substitute with NOW() for MySQL.
                            # MySQL requires DATETIME/TIMESTAMP values to be valid formats.
                            if original_created_at is None or not original_created_at.strip():
                                new_row[created_at_idx] = mysql.connector.FieldType.get_info(mysql.connector.FieldType.TIMESTAMP).encode('utf-8') # Placeholder for NOW()
                            
                            # updated_at will auto-update, but for initial insert, providing original or NOW() is fine.
                            # Forcing NOW() for simplicity if the original is problematic or NULL.
                            if original_updated_at is None or not original_updated_at.strip():
                                new_row[updated_at_idx] = mysql.connector.FieldType.get_info(mysql.connector.FieldType.TIMESTAMP).encode('utf-8') # Placeholder for NOW()

                        except ValueError:
                            # Columns not found, no special handling needed
                            pass
                    
                    processed_rows.append(tuple(new_row))

                placeholders = ','.join(['%s'] * len(original_col_names))
                insert_stmt = f"INSERT INTO `{table_name}` ({','.join(f'`{col}`' for col in original_col_names)}) VALUES ({placeholders})"
                ...
                for i in range(0, len(processed_rows), batch_size):
                    ...
                    # Replace NOW() placeholders with the actual function call for MySQL
                    final_batch = []
                    for row_tuple in batch:
                        converted_row = []
                        for j, value in enumerate(row_tuple):
                            if isinstance(value, bytes) and value == mysql.connector.FieldType.get_info(mysql.connector.FieldType.TIMESTAMP).encode('utf-8'):
                                converted_row.append(mysql.connector.F_NOW) # Use MySQL connector's special NOW() object
                            else:
                                converted_row.append(value)
                        final_batch.append(tuple(converted_row))

                    try:
                        mysql_cursor.executemany(insert_stmt, final_batch)
                    except mysql.connector.Error as err:
    ...
    # --- Configuration ---
sqlite_database_file = 'database/database.sqlite'
mysql_connection_config = {
    'host': '192.168.56.101',
    'user': 'ams_user',
    'password': 'HIKbUxZg19rAazkZ',
